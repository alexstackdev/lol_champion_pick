...
class api extends EventEmitter {
    ...
    _onFileCreated(path) {
        fs.readFile(path, 'utf8')
            .then(data =>{
                [this.client.name, this.client.pid, this.client.port, this.client.token, this.client.protocol] = data.split(':');
                this.client.auth = 'Basic ' + Buffer.from(`riot:${this.client.token}`).toString('base64');
                this._initWebSocket();
                this._initRequest();
                this._initSummoner()
    ...
    _initWebSocket() {
        let socket = new WebSocket(`wss://riot:${this.client.token}@localhost:${this.client.port}`, {
            headers: {
        ...
        return new Promise(((resolve, reject) => {
            this.get("/lol-summoner/v1/current-summoner")
                .then(res=>{
                    ...
                    this.summoner.percentCompleteForNextLevel= res.percentCompleteForNextLevel;
    ...
    getChampionName(){
        return new Promise((resolve, reject) => {
            this.get("/lol-chat/v1/me").then(res=>{
